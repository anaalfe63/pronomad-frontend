...
import { Wallet, CreditCard, FileText, Users, Landmark } from 'lucide-react';

const FinanceHub: React.FC = () => {
    const navigate = useNavigate();
    const { user } = useTenant();
    const APP_COLOR = user?.themeColor || '#10b981'; // Emerald theme for Finance
    ...
    const contentMap: Record<string, { title: string, desc1: string, desc2: string }> = {
        default: {
            ...
            desc1: "Secure, centralized accounting without borders.",
            desc2: "Monitor cash flow, log expenses, and manage payroll."
        },
        ledger: {
            title: "Master Ledger",
            desc1: "View your global financial health in real-time.",
            desc2: "Track all inbound revenue and outbound costs."
        },
        expenses: {
            title: "Expense Management",
            desc1: "Log operational costs, supplier payments, and overhead.",
            desc2: "Keep your margins accurate and transparent."
        },
        invoices: {
            title: "B2B Invoicing",
            desc1: "Generate professional, branded PDF invoices.",
            desc2: "Perfect for corporate clients or large group travel partners."
        },
        payroll: {
            title: "Staff Payroll",
            desc1: "Manage employee base salaries securely.",
            desc2: "Calculate tour guide commissions and process driver stipends."
        }
    };
        ...
        <div className="relative min-h-[85vh] bg-[#fffff] overflow-hidden flex flex-col items-center justify-center p-4 py-20 md:py-0 font-sans">
            ...
            <div className="w-full max-w-xl bg-white rounded-[2.5rem] shadow-[0_20px_50px_-15px_rgba(0,0,0,0.08)] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-500">
                ...
                <div className="relative w-full aspect-[4/3] sm:aspect-[16/10] bg-[#fafafc] overflow-hidden border-b border-slate-50 flex-shrink-0">
                    ...
                    {/* NODE 1: Ledger */}
                    <div 
                        onMouseEnter={() => setActiveFeature('ledger')} onMouseLeave={() => setActiveFeature('default')} onClick={() => navigate('/finance-ledger')}
                    ...
                    {/* NODE 2: Expenses */}
                    <div 
                        onMouseEnter={() => setActiveFeature('expenses')} onMouseLeave={() => setActiveFeature('default')} onClick={() => navigate('/expenses')}
                    ...
                    {/* NODE 3: Invoices */}
                    <div 
                        onMouseEnter={() => setActiveFeature('invoices')} onMouseLeave={() => setActiveFeature('default')} onClick={() => navigate('/invoices')}
                    ...
                    {/* NODE 4: Payroll */}
                    <div 
                        onMouseEnter={() => setActiveFeature('payroll')} onMouseLeave={() => setActiveFeature('default')} onClick={() => navigate('/payroll')}
