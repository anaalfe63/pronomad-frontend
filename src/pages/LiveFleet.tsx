...
const getMarkerIcon = (vehicle: Vehicle, themeColor: string) => {
  ...
  const isIdle = vehicle.status !== 'Active' && vehicle.status !== 'In Transit';
...
const LiveFleet: React.FC = () => {
  ...
  const filteredVehicles = useMemo(() => {
      ...
      return vehicles.filter(v => 
          ...
          (v.tripTitle && v.tripTitle.toLowerCase().includes(lowerQuery))
      );
  }, [vehicles, searchTerm]);
  ...
  const handleReportIncident = (v: Vehicle) => {
      if(window.confirm(`REPORT INCIDENT: Flag vehicle ${v.vehicleId} for safety review?`)) {
          alert("Incident logged to Safety Officer dashboard.");
      }
  };
                      ...
                      <Popup className="rounded-3xl shadow-2xl border-none">
                          <div className="p-2 min-w-[240px]">
                              ...
                              <div className="mb-4">
                                  ...
                                  <p className="text-xs text-slate-500 font-medium flex items-center gap-1"><MapPin size={12} style={{ color: APP_COLOR }}/> {v.location}</p>
                              </div>
                              ...
                              <div className="space-y-2">
                                  <button onClick={() => navigate('/staff', { state: { searchQuery: v.driver } })} className="w-full flex items-center justify-between text-xs font-bold text-slate-700 bg-slate-50 hover:bg-slate-100 transition-colors p-3 rounded-xl border border-slate-100">
                                      <span className="flex items-center gap-2"><User size={14}/> {v.driver}</span>
                                      <ArrowRight size={14}/>
                                  </button>
                                  <button onClick={() => navigate('/operations')} className="w-full flex items-center justify-between text-xs font-bold text-slate-700 bg-slate-50 hover:bg-slate-100 transition-colors p-3 rounded-xl border border-slate-100">
                                      <span className="flex items-center gap-2"><Navigation size={14} className="truncate"/> Trip Manifest</span>
                                      <ArrowRight size={14}/>
                                  </button>
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto min-w-[350px]">
                  <div className="relative flex-1">
                      ...
                      <input 
                          ...
                          placeholder="Search driver, plate, trip..." 
                  ...
                  {selectedVehicleId && (
                      <button onClick={() => setSelectedVehicleId(null)} className="px-6 py-4 bg-slate-200 hover:bg-slate-300 text-slate-600 rounded-2xl text-xs font-bold transition-colors flex items-center justify-center gap-2 whitespace-nowrap">
                      ...
                      >
                          <div className="flex justify-between items-start mb-4">
                              ...
                              {v.speed > 100 ? (
                                  ...
                                  <span className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-xl ${v.status === 'Active' || v.status === 'In Transit' ? 'text-white' : 'bg-slate-100 text-slate-500'}`} style={v.status === 'Active' || v.status === 'In Transit' ? { backgroundColor: APP_COLOR } : {}}>{v.status}</span>
                              )}
                          </div>
                          
                          <div className="flex items-center justify-between text-sm text-slate-500 pt-4 border-t border-slate-100">
                              <div className="flex items-center gap-2 font-bold text-slate-700"><User size={16} style={{ color: APP_COLOR }}/> {v.driver}</div>
